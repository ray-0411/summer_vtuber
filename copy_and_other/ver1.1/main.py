...
def save_viewer_count(channel_name, count, db_path=DB_PATH):
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    now = time.localtime()
    ...
    cursor.execute('''
    INSERT INTO viewers (channel, count, date, time) VALUES (?, ?, ?, ?)
    ''', (channel_name, count, date_str, time_str))
    conn.commit()
    conn.close()
    print(f"✅ 已儲存至資料庫：{channel_name} - {count} 人 ({date_str} {time_str})")
...
def load_channels_from_csv(csv_path):
    ...
    with open(csv_path, newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            ...
            base_url = row['url'].rstrip('/')  # 移除多餘的 / 避免變成 //streams
            full_url = base_url + '/streams'
            channel_list.append((channel_id, channel_name, full_url))
    return channel_list
...
def main(log_callback=None):
    ...
    """
    target_list = [
        ("Yuzumi", "https://www.youtube.com/@%E6%A9%99Yuzumi/streams"),
        ("Rei", "https://www.youtube.com/@%E6%BE%AARei/streams"),
        ("erycyoo", "https://www.youtube.com/@erycyoo/streams"),
    ]
    """
    ...
    # 主程式開始
    for cid, name, url in channel_list:
        
        log(f"\n📡 處理頻道：{name}")
        screenshot_path = f"picture/{cid}_capture.png"
        cropped_path = f"crop/{cid}_crop.png"
        ...
        # 步驟 1：截圖網頁
        if not capture_youtube_screenshot(url, screenshot_path):
            log("❌ 截圖失敗，略過此頻道")
            continue
        ...
        if viewer_count:
            log(f"🎉 [{name}] 正在觀看人數：{viewer_count} 人")
            
            # 儲存到文字檔
            with open("viewer_count.txt", "a", encoding="utf-8") as f:
                f.write(f"[{name}] 觀看人數：{viewer_count} 人\n")
                f.write(f"時間：{time.strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            
            # 儲存到資料庫
            save_viewer_count(name, int(viewer_count))
